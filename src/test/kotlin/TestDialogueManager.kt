...
class TestDialogueManager {
    ...
    fun dialogueBeforeRemove() {
        ...
        manager.dialogues["test"] = Dialogue(RawHttpRequest(null, null, null,null), TestSocket(true), true)
    ...
    fun dialoguePersist() {
        ...
        val manager = DialogueManager(delay)
        manager.startCleaner()
        manager.dialogues["test"] = Dialogue(RawHttpRequest(null, null, null,null), TestSocket(false), true)
        Thread.sleep(delay * 1100)
        Assertions.assertEquals(1, manager.dialogues.size)
        manager.close()
    }
    ...
    fun dialogueCountRemove() {
        ...
        manager.startCleaner()
        for (i in 1..2) {
            manager.dialogues["test$i"] = Dialogue(RawHttpRequest(null, null, null,null), TestSocket(false), true)
        }
        for (i in 3..5) {
            manager.dialogues["test$i"] = Dialogue(RawHttpRequest(null, null, null,null), TestSocket(true), true)
        }
